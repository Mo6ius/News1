...
public final class QueryUtils {
     ...
     */
    private static List<News> extractFeatureFromJson(String response) throws JSONException {
        ...
        try {
            ...
            // Extract the JSONArray associated with the key called "results" (a list of articles).
            JSONArray newsArray = res.getJSONArray("results");

            // For each article in the newsArray, create a currentNews object.
            for (int i = 0; i < newsArray.length(); i++) {
                ...
                // Declare String variable to hold author's name.
                String author = null;

                // Extract the value for the author's name in the tagsArray.
                if (tagsArray.length() == 1) {
                    JSONObject authorTag = (JSONObject) tagsArray.get(0);
                    author = authorTag.getString("webTitle");
                }

                // Create a news object with the title, section, date, and url.
                News news = new News(title, author, section, date, url);

                // Add the news to the list of articles.
                articles.add(news);
            }
